...
export const adicionaContato = email => {
    ...
    return dispatch => {
        ...
        firebase.database().ref(`/contatos/${emailB64}`)
            .once('value')
            .then(snapshot => { //snapshot = chamada no banco
                if(snapshot.val()) { //se snapshot for nulo
                    const dadosUsuario = _.first(_.values(snapshot.val())); //Func recebe obj e retorna array //Func first pq nome do ctt esta salvo no indice 0 do array


                    const { currentUser } = firebase.auth();//email do usuário autenticado
...
export const contatosUsuarioFetch = () => {
    ...
    return (dispatch) => {
        ...
        firebase.database().ref(`/usuario_contatos/${emailUsuarioB64}`)
            .on("value", snapshot => { //semper que houver alteracao no banco, acao é disparada
...
export const deletaConta = () => {
    const { currentUser } = firebase.auth();

    return dispatch => {
        dispatch({ type: DELETANDO_CONTA });

        let usuarioEmailB64 = b64.encode(currentUser.email);

        firebase.database().ref(`/contatos/${usuarioEmailB64}`).remove()
            .then(() => {
                Actions.formLogin();
            })
    }    
